...
**ABC can drive production too.** Three directive forms ride inside the ABC
text: `%%agentscore {json}` applies any track fields (instrument, fx, volume,
transpose…) to the current voice — or song fields (master, swing, custom
instruments) to the song; `%%MIDI program N` maps the standard GM program
directive to the closest AgentScore instrument; and `V:2 name="supersaw"`
sets a voice's instrument by catalog id. So an agent can write notation in
ABC *and* score the production in the same file — see SPEC.md.
...
A track's `instrument` is an id from the catalog (≈110 sounds across 15
...
into slices and re-sequence them — e.g. a breakbeat). The repo bundles four
fully-synthesized, **license-clean** breaks for the slicer to chop
(`samples/breaks/`: `break`, `jungle`, `funk`, `fourfloor` — one bar each on a
16-step grid; generated by `tools/make_breaks.py`). See SPEC.md.

**Drum kits** — three full synthesized kits triggered by name; the duration
sets the step (`kick:4 hat:8 hat:8 snare:4`). Names: kicks `kick kick808 sub`,
snare/clap `snare rim clap`, cymbals `hat openhat ride crash`, toms `tom lowtom
hitom`, hand perc `cowbell shaker tamb conga clave perc`. All three kits answer
to the **same names**, so `"instrument": "kit808"` or `"kit909"` swaps the
whole kit flavor with one word.

**Per-track effects** — give any track an `fx` chain, 20 types: `filter delay
pingpong distortion bitcrush chorus phaser tremolo reverb eq wah autopan
autofilter vibrato pitchshift freqshift chebyshev widener compressor limiter`.
They apply in order between the instrument and the track fader; params are
clamped to safe ranges. Any effect param can be **swept over time straight from
the score** with a `ramp` (`{"param":"frequency","to":6000,"at":0,"len":16}` —
beats), so filter openings, wah sweeps and riser builds are all text. See
SPEC.md for the full tables.

**Track tools** — plain score fields that work in JSON, MD and (via directives)
ABC: `repeat` (loop a pattern N times — huge JSON compression for long songs),
`transpose` (semitones), `offsetBeats` (pickups / delayed entries), `humanize`
(0–1 deterministic timing+velocity jitter — same score always renders the same
bytes).
